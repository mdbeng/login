...
switch ($action) {
  ...
  case 'login':
    // Check for failed login attempts (IP-based blocking)
    if ($cache->enabled()) {
      $tmp = '';
      ...
      $t = $tmp[$client_ip] ?? null;
      
      // Check if IP is currently banned
      if (isset($t) && $t['attempts'] >= 5) {
        sendError('Your account has been blocked for 30 minutes.');
      }
    }
    ...
    if ($cache->enabled() && isset($tmp) && isset($client_ip) && isset($tmp[$client_ip])) {
      unset($tmp[$client_ip]);
      $cache->set('failed_logins', serialize($tmp), 60 * 60);
    }
